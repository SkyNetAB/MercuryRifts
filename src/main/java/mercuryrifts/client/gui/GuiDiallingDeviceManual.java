...
public class GuiDiallingDeviceManual extends BaseGui
{
    ...
    public void initGui()
    {
        ...
        buttonDial = new GuiButton(3, guiLeft + xSize - 87, guiTop + 136, 80, 20, mercuryrifts.localize("gui.dial"));
        buttonDial.enabled = !controller.isPortalActive();
        
        buttonList.add(new GuiButton(0, guiLeft + 7, guiTop + 115, 80, 20, mercuryrifts.localize("gui.clear")));
        buttonList.add(new GuiButton(1, guiLeft + xSize - 87, guiTop + 115, 80, 20, mercuryrifts.localize("gui.save")));
        buttonList.add(new GuiButton(2, guiLeft + 7, guiTop + 136, 80, 20, mercuryrifts.localize("gui.cancel")));
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2)
    {
        ...
        getFontRenderer().drawString(mercuryrifts.localize("gui.uniqueIdentifier"), 7, 18, 0x404040);
        getFontRenderer().drawString(mercuryrifts.localize("gui.glyphs"), 7, 50, 0x404040);
        
        if (warningTimer > 0)
        {
            drawRect(7, 27, 7 + 162, 27 + 18, 0xAA000000);
            String s = mercuryrifts.localize("gui.noUidSet");
    ...
    protected void actionPerformed(GuiButton button)
    {
        ...
        else if (button.id == 1) // save
        {
            if (selector.getGlyphIdentifier().size() > 0)
            {
                ...
                mercuryrifts.packetPipeline.sendToServer(new PacketRequestGui(dial, GuiHandler.DIALLING_DEVICE_C));
            }
            else
            {
                warningTimer = 100;
