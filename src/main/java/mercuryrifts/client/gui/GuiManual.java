...
import mercuryrifts.client.gui.elements.ElementManualCraftingGrid;
import mercuryrifts.client.gui.elements.ElementManualTextButton;
...
public class GuiManual extends BaseGui
{
    ...
    static ResourceLocation textureB = new ResourceLocation("mercuryrifts", "textures/gui/manualB.png");
    ...
    public GuiManual(EntityPlayer p)
    {
        ...
        texture = new ResourceLocation("mercuryrifts", "textures/gui/manualA.png");
    }
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2)
    {
        ...
        if (ClientProxy.manualEntry.equals("main"))
        {
            if (ClientProxy.manualPage == 0)
            {
                int offset = drawSplitString(17, 12, pageWidth, mercuryrifts.localize("manual." + ClientProxy.manualEntry + ".title"), 0x000000);
                getFontRenderer().drawSplitString(mercuryrifts.localize("manual." + ClientProxy.manualEntry + ".page.0"), 17, 15 + offset, pageWidth, 0x000000);
            }
        }
        else if (ClientProxy.manualPage == 0)
        {
            int offset = drawSplitString(17, 12, pageWidth, mercuryrifts.localize("manual." + ClientProxy.manualEntry + ".title"), 0x000000);
            String sub = mercuryrifts.localize("manual." + ClientProxy.manualEntry + ".subtitle");
            ...
            getFontRenderer().drawSplitString(mercuryrifts.localize("manual." + ClientProxy.manualEntry + ".page.0"), 147, 12, pageWidth, 0x000000);
        }
        else
        {
            getFontRenderer().drawSplitString(mercuryrifts.localize("manual." + ClientProxy.manualEntry + ".page." + ClientProxy.manualPage), 15, 12, pageWidth, 0x000000);

            if (ClientProxy.manualEntryHasPage(ClientProxy.manualPage + 1))
            {
                getFontRenderer().drawSplitString(mercuryrifts.localize("manual." + ClientProxy.manualEntry + ".page." + (ClientProxy.manualPage + 1)), 148, 12, pageWidth, 0x000000);
            }
        }
